-- Manual fix for vehicle_model migration
-- Run this directly in MySQL if Liquibase migration fails
...
-- Step 3: Add vehicle_model_id column to vehicles if not exists
SET @column_exists = (
    SELECT COUNT(*) 
...
SET @sql = IF(@column_exists = 0, 
    ...
    'SELECT "Column vehicle_model_id already exists" AS message'
);
...
-- Step 4: Update vehicles to link to vehicle_models
UPDATE vehicles v
...
-- Step 7: Make vehicle_model_id NOT NULL
...
VALUES ('25-fixed', 'antigravity', 'db/changelog/db.changelog-master.yaml', NOW(), 
    (SELECT COALESCE(MAX(ORDEREXECUTED), 0) + 1 FROM (SELECT ORDEREXECUTED FROM DATABASECHANGELOG) AS temp), 
...
ON DUPLICATE KEY UPDATE DATEEXECUTED = NOW();
